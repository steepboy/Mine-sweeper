...
class minesweeper:
    ...
    def click(self, clicked_button):
        ...
        if clicked_button.is_mine:
            ...
            showinfo("Гра Завершена!", "Ви Програли!")
            ...
            if open_count == minesweeper.col * minesweeper.row - minesweeper.mines:
                ...
                minesweeper.is_game_over = True
                showinfo("Гра Завершена!", "Ви Виграли!")
    ...
    def create_settings_win(self):
        ...
        tk.Label(win_settings, text='Рядків').grid(row=0, column=0)
        row_entry = tk.Entry(win_settings)
        row_entry.insert(0, minesweeper.row)
        row_entry.grid(row=0, column=1, padx=20, pady=20)
        tk.Label(win_settings, text='Стовпчиків').grid(row=1, column=0)
        col_entry = tk.Entry(win_settings)
        col_entry.insert(0, minesweeper.col)
        col_entry.grid(row=1, column=1, padx=20, pady=20)
        tk.Label(win_settings, text='Мін').grid(row=2, column=0)
        mines_entry = tk.Entry(win_settings)
        mines_entry.insert(0, minesweeper.mines)
        mines_entry.grid(row=2, column=1, padx=20, pady=20)
        apply_btn = tk.Button(win_settings, text='Зберегти', command=lambda :self.change_settings(row_entry, col_entry, mines_entry))
    ...
    def create_widgets(self):
        ...
        settings_menu.add_command(label='Перезавантажити', command=self.replay)
        settings_menu.add_command(label='Налаштування', command=self.create_settings_win)
        settings_menu.add_command(label='Вихід', command=self.exit_menu)
